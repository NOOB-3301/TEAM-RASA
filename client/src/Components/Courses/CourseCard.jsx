...
export default function CourseCard({ course }) {
  return (
    ...
    >
      ...
      <div className="absolute inset-0 border-2 border-transparent rounded-lg group-hover:border-cyan-500 transition-all duration-300"></div>

      <h2 className="text-xl font-bold text-black">{course.title}</h2>
      <p className="text-gray-600 mt-2">{course.desc}</p>
      
      <div className="mt-4">
        <p className="text-sm text-black font-semibold">Instructor: {course.teacherName}</p>
